...
-- Function to clear only error and warning messages
local function clearErrorAndWarningMessages()
    local DevConsole = game:GetService("CoreGui"):WaitForChild("DevConsoleMaster")
    local DevWindow = DevConsole:WaitForChild("DevConsoleWindow")
    local DevUI = DevWindow:WaitForChild("DevConsoleUI")
    local MainView = DevUI:WaitForChild("MainView")
    ...
    for _, v in pairs(ClientLog:GetChildren()) do
        if v:IsA("GuiObject") and v.Name == v.Name:match("%d+") then
            -- Check if the message text contains "Error" or "Warning"
            local label = v:FindFirstChildOfClass("TextLabel") or v:FindFirstChildOfClass("TextBox")
            if label and (label.Text:find("invalid property") or label.Text:find("attempt")) then
                v:Destroy()
            end
        end
    end
...
if getgenv().CleanConsole.Enabled and getgenv().CleanConsole.Mode == "auto" then
    RunService.Heartbeat:Connect(function()
        if shouldClear then
            if getgenv().CleanConsole.ClearingMode == "error_warning" then
                clearErrorAndWarningMessages()
            else
                clearConsole()
            end
        end
